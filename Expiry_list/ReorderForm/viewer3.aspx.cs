    ...
    public partial class viewer3 : System.Web.UI.Page
    {
        ...
        private void RespondWithData()
        {
            using (SqlConnection conn = new SqlConnection(strcon))
            {
                ...
                whereClause.Append(" AND (status IN ('Reorder Added', 'No Reorder Added')) AND (approved = 'approved')");
        ...
        protected void ApplyFilters_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dt.Rows.Count == 0)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alertNoResults",
                        "Swal.fire('No Results', 'No data found matching the selected filters.', 'info');", true);
                }
        ...
        private void BindGrid(int pageNumber = 1, int pageSize = 100)
        {
            using (SqlConnection conn = new SqlConnection(strcon))
            {
                ...
                if (perm == "edit")
                {
                    queryBuilder.Append(" AND (status IN ('Reorder Added','No Reorder Added')) AND (approved = 'approved')");
                    if (!isHOUser)
                    {
        ...
        private DataTable GetFilteredData()
        {
            ...
            // Permission filter
            if (perm == "edit" || perm == "view" || perm == "super")
            {
                query.Append(" AND (status IN ('Reorder Added','No Reorder Added')) and (approved='approved')");
            }
