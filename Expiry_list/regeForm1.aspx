      ...
      document.addEventListener('DOMContentLoaded', function () {
       ...
       function resetLocationPills() {
        ...
        <%-- function initPermissionToggles() {
             // Attach event handler for future changes (delegated)
            $('#<%= GridView2.ClientID %>').on('change', 'input[type=checkbox].perm-toggle', function () {
                const $targetDiv = $(this).closest('.form-check').next('.permission-options');
                if ($targetDiv.length) {
                    $targetDiv.toggle(this.checked);
                }
            });

             $('#<%= GridView2.ClientID %> input[type=checkbox].perm-toggle').each(function () {
                 const $targetDiv = $(this).closest('.form-check').next('.permission-options');
                 if ($targetDiv.length) {
                     $targetDiv.toggle(this.checked);
                 }
